...
if ! [ $APP_ENV ]; then
  echo -e "${RED}↪ Setup APP_ENV properly!!! ${NO_COLOR}"
  exit -1
fi

if [ -z $APP_SECRET_PASSPHRASE ]; then
  echo -e "↪ Checking for secrets/secrets File $APP_ENV"
...
if [[ $MODE == "pack" ]]; then
  ...
  SECRETS_TO_PACK="src/secrets prisma/.env prisma/.env.development"
  # Create archive
